...
OUT_PLY = 'Registered_Dorm_v3.ply'
OUT_E57 = 'Registered_Dorm_v3.e57'
OUT_PREVIEW = 'Registered_Dorm_v3_preview.ply'
OUT_POSES = 'poses_v3.json'
...
def spanning_tree_init(edges, n, labels):
    """검증겹침이 높은 에지부터 최대신장트리를 만들고 자세를 전파한다.
    헤더 pose 를 전혀 쓰지 않는 게 핵심 (v2 실패 원인)."""
    conf = {}
...
def main():
    ...
    poses_cur = {i: priors[i] for i in range(n)}
    best = None
    for rnd in range(1, args.rounds + 1):
        edges = build_edges(pairs, poses_cur, labels, fine, fnorm, med, mnorm,
                            coarse, trees_f, trees_m, rnd)
        if not edges:
            print("  채택된 에지가 없어 중단")
            break
        print(f"\n--- [라운드 {rnd}] 이상 에지 제거 & 신장트리 초기화 ---")
        edges = filter_by_triangles(edges, labels)
        anchor, tree_poses = spanning_tree_init(edges, n, labels)
        if anchor is None:
            break
        print(f"\n--- [라운드 {rnd}] 포즈그래프 최적화 (prior 없음) ---")
        P = optimize(edges, tree_poses, anchor, labels)

        keep = sorted(P)
        q = layer_quality(full, P, keep, labels)
        print(f"\n--- [라운드 {rnd}] 결과 ---")
        ...
        poses_cur = dict(P)
        for i in range(n):
            poses_cur.setdefault(i, priors[i])
