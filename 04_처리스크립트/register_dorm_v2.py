...
OUT_PLY = 'Registered_Dorm_v2.ply'
OUT_E57 = 'Registered_Dorm_v2.e57'
OUT_PREVIEW = 'Registered_Dorm_v2_preview.ply'
...
# --- 에지 채택 기준 -----------------------------------------------------------
# v1 은 403호처럼 "보정량이 크면 기각"이었는데, 이 데이터셋에서는 그게 틀렸다.
# S003-S004 는 fit 88.6% / rmse 14.2mm 로 전체 최고 품질인데 보정량 757mm 라는
# 이유만으로 기각됐다. 헤더상 두 스캔의 상대 이동량이 정확히 0.757m 인 걸 보면
# ICP 는 "둘이 사실상 같은 자리"라고 본 것이고, 점 수(465k vs 3.39M)를 보면
# 같은 삼각대에서 저해상도->고해상도로 재촬영한 쌍일 가능성이 높다.
# 즉 현장 정합 오차가 dm 급이라 보정량으로 거르면 진짜 보정까지 죽는다.
# -> 보정량 대신 "정합 품질(fit/rmse) + 독립 검증(거친 겹침 증가)"으로 거른다.
# 문턱값은 diag_bigcorr.py 로 17쌍을 실측해서 정했다 (diag_bigcorr.log 참고).
# 진짜 보정으로 확인된 것 중 fit 최저는 S018-S019 의 18.3%,
# 가짜로 판명난 것 중 fit 최고는 S014-S018 의 12.3% 라 그 사이인 15% 로 끊는다.
MIN_PAIR_FITNESS = 0.15     # 최종 스케일에서의 inlier 비율 하한
MAX_PAIR_RMSE = 0.025       # m
SANITY_T = 4.0              # m, 이 이상은 상식적으로 불가 -> 기각
SANITY_R = 20.0             # deg
VERIFY_BIG_T = 0.20         # m, 이보다 큰 보정은 독립 검증을 거친다
VERIFY_BIG_R = 2.0          # deg
VERIFY_GAIN = 1.5           # 거친 겹침이 최소 이 배수로 늘어야 인정
VERIFY_MIN_OV = 0.20        # 그리고 절대값도 이 이상이어야 인정
                            # (가짜 S015-S018 은 14.4% 에 그쳐 여기서 걸린다)

# --- 포즈 그래프 --------------------------------------------------------------
PRIOR_WEIGHT = 0.10         # 헤더 pose 로 끌어당기는 힘. dm 급 오차가 있으므로
                            # 403호(0.3)보다 약하게 잡는다.
ROBUST_LOSS = 'soft_l1'     # 남은 불량 에지를 자동 감쇠
ROBUST_FSCALE = 0.05
...
# ---------------------------------------------------------------- 정합
def mutual_coarse_overlap(ca, cb, T_rel, thresh=COARSE_THRESH):
    """거친 클라우드로 양방향 겹침 평균. ICP 가 최적화한 값이 아니므로
    대형 보정의 진위를 가리는 독립 지표로 쓸 수 있다."""
...
def refine(clouds, normals, coarse, priors, pairs, labels, free_prior_idx=()):
    """후보 쌍 point-to-plane ICP + 로버스트 포즈그래프 최적화."""
    n = len(clouds)
    used = sorted({i for i, j, _ in pairs} | {j for i, j, _ in pairs})
    ...
    for k, (i, j, ov) in enumerate(pairs):
        ...
        quality = (fit > MIN_PAIR_FITNESS and np.isfinite(rmse)
                   and rmse < MAX_PAIR_RMSE)
        sane = dt < SANITY_T * 1000 and dr < SANITY_R

        note = ""
        ok = quality and sane
        # 보정이 크면 ICP 자기 채점만으로는 못 믿는다 -> 거친 클라우드로 재검증
        if ok and (dt > VERIFY_BIG_T * 1000 or dr > VERIFY_BIG_R):
            n_big += 1
            ov0 = mutual_coarse_overlap(coarse[i], coarse[j], T_prior)
            ov1 = mutual_coarse_overlap(coarse[i], coarse[j], T)
            ok = ov1 > ov0 * VERIFY_GAIN and ov1 > VERIFY_MIN_OV
            note = (f" 검증 겹침 {ov0*100:.1f}%->{ov1*100:.1f}%"
                    f"{'' if ok else ' 검증실패'}")
        elif not sane:
            note = " 상식범위초과"
        elif not quality:
        ...
        print(f"  [{k+1:>2}/{len(pairs)}] {labels[i]}-{labels[j]} "
              ...
              f"보정 dt={dt:7.1f}mm dr={dr:5.2f}deg  "
              f"[{'채택' if ok else '기각(사전값 유지)'}]{note}", flush=True)
        edges[(i, j)] = (T if ok else T_prior, fit if ok else 0.3)
    print(f"  => 채택 {n_ok}/{len(pairs)}쌍 (대형 보정 독립검증 {n_big}건)")
    ...
    def residuals(x):
        ...
        for k in free:
            ...
            w = PRIOR_WEIGHT * (0.2 if k in free_prior_idx else 1.0)
            r.append(se3_log(np.linalg.inv(B[k]) @ P[k]) * w)
        return np.concatenate(r)

    # 로버스트 손실(soft_l1): 채택 기준을 통과했지만 여전히 다른 에지들과
    # 모순되는 에지(순환 불일치)를 자동으로 감쇠시킨다. 'lm' 은 손실 함수를
    # 지원하지 않으므로 'trf' 를 쓴다.
    x0 = np.zeros(len(free) * 6)
    sol = least_squares(residuals, x0, method='trf', loss=ROBUST_LOSS,
                        f_scale=ROBUST_FSCALE, xtol=1e-12, ftol=1e-12,
                        max_nfev=20000)
    print(f"  수렴: {sol.success} | 잔차 norm {np.linalg.norm(sol.fun):.6f} "
          f"| 손실 {ROBUST_LOSS}(f_scale={ROBUST_FSCALE})")

    poses = poses_from_x(sol.x)

    # 에지별 최종 잔차 -> 어떤 에지가 그래프와 모순되는지 보고
    resid = []
    for (i, j), (T_ij, w) in edges.items():
        e = se3_log(np.linalg.inv(poses[j] @ T_ij) @ poses[i])
        resid.append((np.linalg.norm(e[3:]) * 1000, labels[i], labels[j]))
    resid.sort(reverse=True)
    print("  에지 잔차 상위 5개 (mm): " +
          ", ".join(f"{a}-{b} {v:.0f}" for v, a, b in resid[:5]))
    for k in range(n):
...
def main():
    ...
    if args.prior_only or not pairs:
        ...
        poses = refine(clouds, normals, coarse, priors, pairs, labels,
